...
自定义登录入口保存在 SQLite 数据库的 `settings` 表中。为避免隐藏路径被泄露，公开落地页不提供找回链接。

在项目根目录查看当前登录入口：

```bash
sqlite3 data/outlook_accounts.db \
  "SELECT value FROM settings WHERE key = 'login_entry_path';"
```

如果本机没有 `sqlite3` 命令，可使用项目 Python 环境：

```bash
venv/bin/python -c "import sqlite3; db=sqlite3.connect('data/outlook_accounts.db'); print(db.execute(\"SELECT value FROM settings WHERE key='login_entry_path'\").fetchone()[0])"
```
...
```bash
sqlite3 data/outlook_accounts.db \
  "UPDATE settings SET value = '/login' WHERE key = 'login_entry_path';"
```

数据库修改后通常会立即生效，无需重启服务。如果应用使用了自定义 `DATABASE_PATH`，请将命令中的 `data/outlook_accounts.db` 替换为实际数据库路径。
